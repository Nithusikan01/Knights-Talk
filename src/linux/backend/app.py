...
try:
    ...
    recognizer = None
    microphone = None
    print("Microphone not available, will use text input.", e)
...
def capture_speech(prompt):
    ...
    if recognizer is None or microphone is None:
        return None  # Return None if speech recognition is not available
...
def get_human_move():
    ...
    """
    while True:
        move = capture_speech("Please say your move: ") if microphone else None
        if move is None:
            move = input("Please type your move (e.g., e2 e4): ")  # Fallback to text input
        speak(move)
        if move:
